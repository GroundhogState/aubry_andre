%% Bonus
% An animated feature that shows which elements are summed over to produce
% elements of the reduced density matrix. Don't take it too seriously, but
% gives some visual intuition for the process.
%     

rho = Tensor(rand_qubit,rand_qubit,rand_qubit);
dim = [2 2 2];
sys = 2;
...
dim_out = d;
rho = zeros(dim_out); 
left_keep = min(tr_keep);
right_keep = max(tr_keep);
Q = zeros(dim_out);
%%%%%%%%%
% A partial trace demo.
% Accepts one density matrix or state vector (rho),  
%       an array of dimensions of the subsystems (dim), 
%       and an array of subsystems to trace out (sys)
%
...
%   - Trace out contiguous internal blocks early
%   - Convert to a general expression that reduces matrix in a single
...
%% Function body
% A simple optimization: If there is a large contiguous block on either end,
